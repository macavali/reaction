...
declare agents =
  ...
  where
    ...
    mkagent :: [(Text, Text, Maybe StateP)] -> Statement
    mkagent a =
      let (name, _, _):_ = a in
...
sitesD :: Statement -> [Text]
sitesD (AgentD _ sp) = H.keys sp
sitesD _ = undefined

statesD :: Text -> Statement -> [Text]
statesD x (AgentD _ s) =
  case H.lookup x s of
   Just st -> st
   Nothing -> []
statesD _ _ = undefined

fillSites :: Statement -> AgentP -> AgentP
...
fillSites _ _ = undefined

enumState :: Text -> Statement -> AgentP -> [AgentP]
...
enumState' :: String -> Statement -> AgentP -> [AgentP]
enumState' x = enumState (pack x)

enumStates :: Statement -> AgentP -> [AgentP]
